...
export const MasterDataPage: React.FC = () => {
  const { modalProduk, modalLogo, margin, brands, users, divisi, keterangan } = useMasterData();
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {activeTab === 'users' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card className="p-5">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3">Daftar Sales Rep</h4>
            <div className="divide-y divide-slate-100 dark:divide-slate-800 text-xs">
              {users.map((u: UserSales) => (
                <div key={u.id} className="py-2.5 flex items-center justify-between">
                  <span className="font-semibold text-slate-900 dark:text-slate-100">{u.nama}</span>
          ...
          <Card className="p-5">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3">Daftar Divisi</h4>
            <div className="divide-y divide-slate-100 dark:divide-slate-800 text-xs">
              {divisi.map((d: Divisi) => (
                <div key={d.id} className="py-2.5 flex items-center justify-between">
                  <span className="font-semibold text-slate-900 dark:text-slate-100">{d.nama_divisi}</span>
